    ...
    <div class="max-w-md mx-auto space-y-6">
        <div class="w-full max-w-md space-y-8">
            ...
            <form action="" method="POST">
                ...
                <div class="flex justify-center gap-2 sm:gap-4 my-8">
                    @for ($i = 0; $i < 6; $i++)
                        <input type="text" name="otp[]" maxlength="1" inputmode="numeric" pattern="[0-9]*"
                            autocomplete="off"
                            class="otp-input w-12 h-12 sm:w-16 sm:h-16 text-center text-2xl font-bold border-2 border-gray-300 rounded-lg focus:border-gray-500 focus:outline-none">
                    @endfor
                </div>
            ...
            <!-- Resend OTP Form (Separate from OTP Form) -->
            <form id="resendForm" action="{{ route('subscriptionLoginEmail') }}" method="POST">
                @csrf
                <input type="hidden" name="email" value="{{ $email }}">

                <!-- Resend Button -->
                <div class="flex justify-center">
                    <button type="submit" class="flex items-center gap-2 text-gray-900 hover:text-gray-700">
